...
from conbo.sampling import uniform_sampling, lhs_sampling
from conbo.gpr import GPR, GPRSkeleton, InternalGPR
...
def test_GPR_incorrect_input_shape_fitting(rng: random.Generator):
...
def test_GPR_incorrect_output_shape_fitting(rng: random.Generator):
    gpr_model = InternalGPR()
    gpr = GPR(gpr_model)
    def internal_function(X):
        return X[:,0] ** 2 + X[:,1] ** 2 + X[:,2] ** 2
...
def test_GPR_inconsistent_iodat_fitting(rng: random.Generator):
    gpr_model = InternalGPR()
    gpr = GPR(gpr_model)
    def internal_function(X):
        return X[:,0] ** 2 + X[:,1] ** 2 + X[:,2] ** 2
...
def test_GPR_inconsistent_input_prediction(rng: random.Generator):
    gpr_model = InternalGPR()
    gpr = GPR(gpr_model)
    def internal_function(X):
        return X[:,0] ** 2 + X[:,1] ** 2 + X[:,2] ** 2
...
def test_GPR_output_prediction(rng: random.Generator, data_path: pathlib.Path):
    gpr_model = InternalGPR()
    gpr = GPR(gpr_model)
    def internal_function(X):
        return X[:,0] ** 2 + X[:,1] ** 2 + X[:,2] ** 2
